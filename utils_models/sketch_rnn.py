...
class sketch_rnn:
    def __init__(self, params):
        ...
        :param params: All parameters needed is saved in this
        """
        self.params = params
        self.pi = math.pi
    ...
    def get_rnn_layers(self):
        ...
        with tf.variable_scope("encoder"):
            ...
            if self.params.bidir:
                rnn_out, _ = self.get_bi_rnn(cell)
            else:
                rnn_out, _ = self.get_basic_rnn(cell)

        with tf.variable_scope("z"):
            z = self.get_z(rnn_out)
    ...
    def get_z(self, inputs):
        ...
        w_theta = tf.get_variable("w_theta", [self.params.num_r_n * bidir, self.params.dim_z], self.params.d_type)
        b_theta = tf.get_variable("b_theta", [self.params.dim_z], self.params.d_type)
    ...
    def get_decoder(self, z):
        ...
        def _train_with_inputs_helper():
            """
            The inputs has been concatenated with z
        ...
        decoder = tf.contrib.seq2seq.BasicDecoder(cell=de_cells,
                                                  helper=helper,
                                                  initial_state=tf.contrib.rnn.LSTMStateTuple(c0, h0),
